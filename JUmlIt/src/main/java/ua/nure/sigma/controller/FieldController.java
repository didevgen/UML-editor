...
public class FieldController {
	...
	public ResponseEntity<Void> removeField(@PathVariable long diagramId, @PathVariable long classId, @PathVariable long fieldId,
			Principal principal) {
		if (!UserAccessibility.hasAccess(principal, diagramId)) {
	...
	public ResponseEntity<Field> updateField(@RequestBody Field field, @PathVariable long diagramId, @PathVariable long classId,
			Principal principal) {
		if (!UserAccessibility.hasAccess(principal, diagramId)) {
			return new ResponseEntity<Field>(HttpStatus.FORBIDDEN);
		}
